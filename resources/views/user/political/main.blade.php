@extends('user.political.layout')

@section('content')

    <!--? slider Area Start-->
    <div class="slider-area ">
        <div class="slider-active">
            <!-- Single Slider -->
            <div class="single-slider slider-height d-flex align-items-center">
                <div class="container">
                    <div class="row align-items-center"  id="home_container">
                        <div class="col-xl-6 col-lg-7 col-md-8">
                            <div class="hero__caption">
                                <span data-animation="fadeInLeft" data-delay=".2s">
                                    <img src="{{ asset('images/political/front_end/Logov2.png') }}" class="fade_img" alt="" style="height: 180px; width:auto">
                                </span>
                                <h1 data-animation="fadeInLeft" data-delay=".4s" style="font-size: 30px">{{trans('user.organization_control')}}</h1>
                                <!-- Hero-btn -->
                                <div class="hero__btn">
                                    <a href="#" class="btn hero-btn login"  data-animation="fadeInLeft" data-delay=".8s">{{trans('user.login')}}</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-5 col-lg-4 margin-top-mobile">
                            <div class="f-left" data-animation="fadeInRight" data-delay=".4s">
                                <img src="{{ asset('images/political/front_end/boxes.jpg') }}" alt="" width="500px" style="margin-top: 400px">
                                <div class="row hidden-mobile" style="margin-top: -60px; color: white; font-size: 13px; text-align: center">
                                    <div class="col-md-3" style="margin-left: 35px">{{trans('user.control_of_your_political_organization')}}</div>
                                    <div class="col-md-3" style="margin-left: 30px">{{trans('user.quality_of_the_information')}}</div>
                                    <div class="col-md-3" style="margin-left: 35px">{{trans('user.security_of_your_data')}}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!-- Single Slider -->
            <div class="single-slider slider-height d-flex align-items-center">
                <div class="container">
                    <div class="row align-items-center"  id="home_container">
                        <div class="col-xl-6 col-lg-7 col-md-8">
                            <div class="hero__caption">
                                <span data-animation="fadeInLeft" data-delay=".2s">
                                    <img src="{{ asset('images/political/front_end/Logov2.png') }}" class="fade_img" alt="" style="height: 180px; width:auto">
                                </span>
                                <h1 data-animation="fadeInLeft" data-delay=".4s" style="font-size: 30px">{{trans('user.organization_control')}}</h1>
                                <!-- Hero-btn -->
                                <div class="hero__btn">
                                    <a href="#" class="btn hero-btn"  data-animation="fadeInLeft" data-delay=".8s">{{trans('user.sign_up')}}</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-5 col-lg-4 margin-top-mobile">
                            <div class="f-left" data-animation="fadeInRight" data-delay=".4s">
                                <img src="{{ asset('images/political/front_end/boxes.jpg') }}" alt="" width="500px" style="margin-top: 400px">
                                <div class="row hidden-mobile" style="margin-top: -60px; color: white; font-size: 13px; text-align: center">
                                    <div class="col-md-3" style="margin-left: 35px">{{trans('user.control_of_your_political_organization')}}</div>
                                    <div class="col-md-3" style="margin-left: 30px">{{trans('user.quality_of_the_information')}}</div>
                                    <div class="col-md-3" style="margin-left: 35px">{{trans('user.security_of_your_data')}}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- slider Area End-->
    <!--? Categories Area Start -->
    <div class="categories-area section-padding30" id="organization_values">
        <div class="container">
            <div class="row justify-content-sm-center" >
                <div class="cl-xl-7 col-lg-8 col-md-10">
                    <!-- Section Tittle -->
                    <div class="section-tittle text-center mb-70">
                        <span>{{trans('user.our_values')}}</span>
                        <h2 style="font-size: 25px">{{trans('user.integral_party_provides_your_political_organization')}}</h2>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-cat mb-50">
                        <div class="cat-icon">
                            <span class="flaticon-web-design"></span>
                        </div>
                        <div class="cat-cap">
                            <h5><a  href="#">{{trans('user.control_functions')}}</a></h5>
                            <p>{!! trans('user.control_functions_text') !!}</p>
                            <a href="#" class="read-more1"> {!! trans('user.read_more') !!} ></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-cat mb-50">
                        <div class="cat-icon">
                            <span class="flaticon-education"></span>
                        </div>
                        <div class="cat-cap">
                            <h5><a  href="#">{{trans('user.information_quality')}}</a></h5>
                            <p>{!! trans('user.information_quality_text') !!}</p>
                            <a  href="#" class="read-more1">{!! trans('user.read_more') !!} ></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-cat mb-50">
                        <div class="cat-icon">
                            <span class="flaticon-communications"></span>
                        </div>
                        <div class="cat-cap">
                            <h5><a  href="#">{{trans('user.communications_and_news')}}</a></h5>
                            <p>{!! trans('user.communications_and_news_text') !!}</p>
                            <a  href="#" class="read-more1">{!! trans('user.read_more') !!} ></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-cat mb-50">
                        <div class="cat-icon">
                            <span class="flaticon-computing"></span>
                        </div>
                        <div class="cat-cap">
                            <h5><a  href="#">{{trans('user.vote_promotion')}}</a></h5>
                            <p>{!! trans('user.vote_promotion_text') !!}</p>
                            <a  href="#" class="read-more1">{!! trans('user.read_more') !!} ></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-cat mb-50">
                        <div class="cat-icon">
                            <span class="flaticon-tools-and-utensils"></span>
                        </div>
                        <div class="cat-cap">
                            <h5><a  href="#">{{trans('user.vote_defense')}}</a></h5>
                            <p>{!! trans('user.vote_defense_text') !!}</p>
                            <a  href="#" class="read-more1">{!! trans('user.read_more') !!} ></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-cat mb-50">
                        <div class="cat-icon">
                            <span class="flaticon-business"></span>
                        </div>
                        <div class="cat-cap">
                            <h5><a  href="#">{{trans('user.party_structure')}}</a></h5>
                            <p>{!! trans('user.party_structure_text') !!}</p>
                            <a  href="#" class="read-more1">{!! trans('user.read_more') !!} ></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Categories Area End -->
    <!--? Count Down Start -->
    <div class="count-down-area pt-50 pb-60 section-bg" id="statistics" style="margin-top: -10px" data-background="{{ asset('images/political/gallery/section_bg01.png') }}">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-12 col-md-12">
                    <div class="count-down-wrapper" >
                        <div class="row justify-content-between">
                            <div class="col-lg-3 col-md-6 col-sm-6">
                                <!-- Counter Up -->
                                <div class="single-counter text-center">
                                    <svg  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="72px" height="69px">
                                        <image  x="0px" y="0px" width="72px" height="69px"  xlink:href="data:img/png;base64," />
                                    </svg>
                                    <span class="counter color-green">{{$fed_district}}</span>
                                    <p class="color-green">{{trans('admin.fed_district')}}</p>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6 col-sm-6">
                                <!-- Counter Up -->
                                <div class="single-counter text-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="72px" height="79px">
                                        <image  x="0px" y="0px" width="72px" height="79px"  xlink:href="data:img/png;base64," />
                                    </svg>
                                    <span class="counter color-blue">{{$loc_districts}}</span>
                                    <p class="color-blue">{{trans('admin.loc_district')}}</p>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6 col-sm-6">
                                <!-- Counter Up -->
                                <div class="single-counter text-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="72px" height="71px">
                                        <image  x="0px" y="0px" width="72px" height="71px"  xlink:href="data:img/png;base64," />
                                    </svg>
                                    <span class="counter color-green">{{$areas}}</span>
                                    <p class="color-green">{{trans('admin.area')}}</p>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6 col-sm-6">
                                <!-- Counter Up -->
                                <div class="single-counter text-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="72px" height="69px">
                                        <image  x="0px" y="0px" width="72px" height="69px"  xlink:href="data:img/png;base64," />
                                    </svg>
                                    <span class="counter color-red">{{$zones}}</span>
                                    <p class="color-red">{{trans('admin.zone')}}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Count Down End -->
    <!--? Team Ara Start -->
   {{-- <div class="team-area pt-160 pb-160 section-bg" id="our_team" style="padding-top: 50px" data-background="{{ asset('images/political/gallery/section_bg02.png') }}">
        <div class="container">
            <div class="row justify-content-center">
                <div class="cl-xl-7 col-lg-8 col-md-10">
                    <!-- Section Tittle -->
                    <div class="section-tittle section-tittle2 text-center mb-70">
                        <span>{{trans('user.group_of_work')}}</span>
                        <h2>{{trans('user.our_team')}}</h2>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-team mb-30">
                        --}}{{--<div class="team-img">
                            <img src="{{ asset('images/political/gallery/team1.png') }}" alt="">
                            <!-- Blog Social -->
                            <ul class="team-social">
                                <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fas fa-globe"></i></a></li>
                            </ul>
                        </div>--}}{{--
                        <div class="team-caption">
                            <h3><a href="#">Ing. Yordan E. Estrada Rodriguez</a></h3>
                            --}}{{--<p>Faculty</p>--}}{{--
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-team mb-30">
                        --}}{{-- <div class="team-img">
                             <img src="{{ asset('images/political/gallery/team2.png') }}" alt="">
                             <!-- Blog Social -->
                             <ul class="team-social">
                                 <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                 <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                 <li><a href="#"><i class="fas fa-globe"></i></a></li>
                             </ul>
                         </div>--}}{{--
                        <div class="team-caption">
                            <h3><a href="#">Msc. Joam Manuel Rincom &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></h3>
                            --}}{{--<p>Faculty</p>--}}{{--
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-6">
                    <div class="single-team mb-30">
                        --}}{{--<div class="team-img">
                            <img src="{{ asset('images/political/gallery/team3.png') }}" alt="">
                            <!-- Blog Social -->
                            <ul class="team-social">
                                <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fab fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fas fa-globe"></i></a></li>
                            </ul>
                        </div>--}}{{--
                        <div class="team-caption">
                            <h3><a href="#">Dr. Efrén David Gutiérrez Casas</a></h3>
                            --}}{{--<p>Faculty</p>--}}{{--
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>--}}
    <!-- Team Ara End -->
    <div class="slider-area " id="about_us" >
        <div>
            <!-- Single Slider -->
            <div class="d-flex align-items-center" style="height: 600px">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-xl-6 col-lg-7 col-md-8" style="margin-top: 50px; text-align: left">
                            <div style="color: grey">{{trans('user.know_our_company')}}</div>
                            <div class="hero__caption" style="padding-top: 40px !important; margin-bottom: 30px">
                                <h1 class="col-md-12" style="font-size: 25px; padding-left: 0px !important; margin-left:0px; margin-bottom: 5px">{{trans('user.know_our_objectives')}}</h1>
                            </div>
                            <p style="text-align: justify; line-height:20px">{{trans('user.about_text')}}</p>
                            <div style="margin-top: 20px; margin-bottom: 20px; color: #033454">
                                <div><i class="fa fa-sun"></i> {{trans('user.quality_of_the_information')}}</div>
                                <div><i class="fa fa-lock"></i> {{trans('user.security_of_your_data')}}</div>
                                <div><i class="fa fa-graduation-cap"></i> {{trans('user.segmented_masive_communication')}}</div>
                                <div><i class="fa fa-book"></i> {{trans('user.party_organization')}}</div>
                            </div>
                            <div class="hero__btn">
                                <a href="#" class="btn hero-btn">{{trans('user.contact_us')}}</a>
                            </div>
                        </div>
                        <div class="col-xl-5 col-lg-4 hidden-mobile">
                            <div>
                                <img src="{{ asset('images/political/front_end/backgrounda.jpg') }}" alt="" width="600px">
                                <div style=" margin-top: -120px; position: absolute">
                                    <div style="text-align: center; margin-left: 150px;">
                                        <h1  style="font-size: 20px; color: #033454;"><strong>{{trans('user.organization_control')}}</strong></h1>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

@endsection