...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('BoxController','importData'))
                <a href="#" data-href="{{ route('box.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
        ...
        <tr>
            ...
            <th>{{trans('admin.box_type')}}</th>
            <th>{{trans('admin.box_index')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('BoxController','create'))
        <a href="{{ route('box.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_box')}}">
