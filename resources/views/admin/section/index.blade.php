...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('SectionController','importData'))
                <a href="#" data-href="{{ route('section.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('SectionController','exportData'))
        ...
        <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
            ...
            <tr>
                ...
                <th>{{trans('admin.section_type')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('SectionController','create'))
        <a href="{{ route('section.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_section')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                section_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('section.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
