   ...
   {{-- end filter--}}
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('PersonController','importData'))
                <a href="#" data-href="{{ route('person.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('PersonController','exportData'))
        ...
        <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
                <thead>
                    <tr>
                        ...
                        <th style="width: 30%">{{trans('admin.name')}}</th>
    ...
    <br/>
   @if(auth()->user()->get_Permission('PersonController','create'))
        <a href="{{ route('person.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_person')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                person_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $('.exportData').click(function(e){
                    ...
                    $.each(its, function( i, v ) {
                        items.push(v.id_persons);
                    });
                    var type = $(this).data('type');
                    var url = '{{ route('person.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
