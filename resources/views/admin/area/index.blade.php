...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('AreaController','importData'))
                <a href="#" data-href="{{ route('area.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('AreaController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('AreaController','create'))
        <a href="{{ route('area.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_group')}}">
...
@section('script_down')
    <script type="text/javascript">
            ...
            $(window).on('load',function(){
                ...
                area_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
