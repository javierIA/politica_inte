...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('MunicipalityController','importData'))
                <a href="#" data-href="{{ route('municipality.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('MunicipalityController','exportData'))
    ...
    <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
        ...
        <tr>
            ...
            <th>{{trans('admin.municipality_key')}}</th>
            <th>{{trans('admin.municipality_name')}}</th>
            <th>{{trans('admin.titular_person')}}</th>
            <th>{{trans('admin.vocal_person')}}</th>
            <th>{{trans('admin.representative')}}</th>
            <th>{{trans('admin.alternate')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('MunicipalityController','create'))
        <a href="{{ route('municipality.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_municipality')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var municipality_table = $("#table-2");
                ...
                municipality_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    municipality_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('municipality.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
