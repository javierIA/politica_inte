...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('UserController','importData'))
                <a href="#" data-href="{{ route('user.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('UserController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('UserController','create'))
        <a href="{{ route('user.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_user')}}">
        ...
        (function($){
            $(window).on('load',function(){
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('user.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
