...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('LocDistrictController','importData'))
                <a href="#" data-href="{{ route('fed_district.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
    ...
    @if(auth()->user()->get_Permission('LocDistrictController','create'))
        <a href="{{ route('fed_district.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_fed_district')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var fed_district_table = $("#table-2");

                // Initialize DataTable
                fed_district_table.DataTable( {
                    "language":{
                ...
                fed_district_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    fed_district_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('fed_district.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
