...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('PoliticalFunctionController','importData'))
                <a href="#" data-href="{{ route('political_function.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('PoliticalFunctionController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('PoliticalFunctionController','create'))
        <a href="{{ route('political_function.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_political_function')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var political_function_table = $("#table-2");

                // Initialize DataTable
                political_function_table.DataTable( {
                    "language":{
                    ...
                    "serverSide": true,
                    "ajax":{
                        "url": "{{route('political_function.filter')}}",
                        "dataType": "json",
                        "type": "POST",
                ...
                political_function_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    political_function_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('political_function.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
