...
@section('breadcrumb')
    ...
    <li>
        <a href="{{ route('loc_district.index') }}">{{ trans('admin.loc_district') }}</a>
    </li>
    <li class="active">
        <strong>{{$title}}</strong>
    </li>
...
@section('content')
    <form role="form" class="form-horizontal form-groups-bordered" method="post" action="{{ route('loc_district.update', $locDistrict) }}" enctype="multipart/form-data">
        ...
        <div class="form-group{{ $errors->any() ? ' has-error' : '' }}">
            <div class="col-md-3 " style="margin-top: -10px; padding-top: 10px">
                ...
                <div class="rcorners" style="height: 252px; overflow-y: auto">
            ...
            <div class="col-md-8">
                ...
                <div class="panel minimal" >
                    ...
                    <div class="panel-heading">
                        <div class="panel-title roboto">{!! trans('admin.loc_district') !!}</div>
                        <div class="panel-options">
                            <ul class="nav nav-tabs">
                                <li class="active" data-name="{!! trans('admin.loc_district') !!}">
                    ...
                    <div class="panel-body">
                        <div class="tab-content">
                            <div class="tab-pane active" id="personal_data_tab" name="{{trans('admin.loc_district')}}">
                                <div class="rcorners" style="margin-top: 10px">
                                    <div class="verticalLine">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="number" class="form-control make_user recorder" max="9999" min="0" name="district_number" id="district_number" value="{{$locDistrict->district_number}}" required autofocus/>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-sm-9">
                                                <select id="fed_entity_id" class="form-control select2 recorder personal_data" required>
                                                    ...
                                                    @foreach($fed_entity as $f)
                                                        <option value="{{ $f->id }}" @if($locDistrict->municipality->fed_entity->id == $f->id) selected @endif>{{ $f->entity_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-sm-9">
                                                <select id="municipality_id" name="municipality_id" class="form-control select2 recorder personal_data" required>
                                                    ...
                                                    @foreach($municipality as $m)
                                                        <option value="{{ $m->id }}" data-parent="{{$m->fed_entity_id}}" @if($locDistrict->municipality_id == $m->id) selected @endif>{{ $m->municipality_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="verticalLine" style="margin-top: 10px">
                                        <div class="row" style="margin-top: 5px">
                                            ...
                                            <div class="col-sm-9">
                                                ...
                                                @if(!is_null($locDistrict->map_pdf))
                                                    <a href="{{ url('pdf/'.$locDistrict->getTable().'/'.basename($locDistrict->map_pdf)) }}" add target="_blank" class="d-flex align-items-center justify-content-center login"><span ><i class="fa fa-map fa-lg"> {{basename($locDistrict->map_pdf)}}</i></span></a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
