...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('StreetController','importData'))
                <a href="#" data-href="{{ route('street.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
    ...
    <br/>
    @if(auth()->user()->get_Permission('StreetController','create'))
        <a href="{{ route('street.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_street')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                street_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    street_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('street.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
