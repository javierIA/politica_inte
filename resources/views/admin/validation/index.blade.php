...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('ValidationController','importData'))
                <a href="#" data-href="{{ route('validation.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('ValidationController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('ValidationController','create'))
        <a href="{{ route('validation.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_validation')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                validation_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    validation_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('validation.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
