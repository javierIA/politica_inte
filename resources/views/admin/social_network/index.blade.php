...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('SocialNetworkController','importData'))
                <a href="#" data-href="{{ route('social_network.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('SocialNetworkController','exportData'))
        ...
        <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
            <thead>
                <tr>
                    ...
                    <th>{{trans('admin.name_social_network')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('SocialNetworkController','create'))
        <a href="{{ route('social_network.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_social_network')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var social_network_table = $("#table-2");
                ...
                social_network_table.DataTable( {
                    ...
                    "serverSide": true,
                    "ajax":{
                        "url": "{{route('social_network.filter')}}",
                        "dataType": "json",
                        "type": "POST",
                ...
                social_network_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    social_network_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('social_network.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
