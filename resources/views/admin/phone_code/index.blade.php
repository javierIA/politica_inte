...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('PhoneCodeController','importData'))
                <a href="#" data-href="{{ route('phone_code.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('PhoneCodeController','exportData'))
    ...
    <br/>
        ...
        <a href="{{ route('phone_code.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_phone_code')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var phone_code_table = $("#table-2");

                // Initialize DataTable
                phone_code_table.DataTable( {
                    "language":{
                ...
                phone_code_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    phone_code_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('phone_code.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
