...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('RoleController','importData'))
                <a href="#" data-href="{{ route('role.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('RoleController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('RoleController','create'))
        <a href="{{ route('role.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_role')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                role_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('role.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
