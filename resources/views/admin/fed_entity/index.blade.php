...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('FedEntityController','importData'))
                <a href="#" data-href="{{ route('fed_entity.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('FedEntityController','exportData'))
        ...
        <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
            ...
            <tr>
                ...
                <th>{{trans('admin.entity_key')}}</th>
                <th>{{trans('admin.entity_name')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('FedEntityController','create'))
        <a href="{{ route('fed_entity.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_fed_entity')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var fed_entity_table = $("#table-2");

                // Initialize DataTable
                fed_entity_table.DataTable( {
                    "language":{
                ...
                fed_entity_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    fed_entity_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('fed_entity.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
