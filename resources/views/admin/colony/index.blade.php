...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('ColonyController','importData'))
                <a href="#" data-href="{{ route('colony.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
    ...
    @if(auth()->user()->get_Permission('ColonyController','create'))
        <a href="{{ route('colony.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_colony')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var colony_table = $("#table-2");

                // Initialize DataTable
                colony_table.DataTable( {
                    "language":{
                ...
                colony_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
            ...
            // start filter part
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    colony_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('colony.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
