...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('ZoneController','importData'))
                <a href="#" data-href="{{ route('zone.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('ZoneController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('ZoneController','create'))
        <a href="{{ route('zone.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_zone')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                zone_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('zone.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
