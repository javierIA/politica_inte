...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('BlockController','importData'))
                <a href="#" data-href="{{ route('block.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('BlockController','exportData'))
        ...
        <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
            ...
            <tr>
                ...
                <th>{{trans('admin.titular')}}</th>
                <th>{{trans('admin.vocal')}}</th>
                <th>{{trans('admin.section')}}</th>
                <th style="width: 15%;">{{trans('admin.actions')}}</th>
            </tr>
            </thead>
        </table>
    </div>
    <br/>
    @if(auth()->user()->get_Permission('BlockController','create'))
        <a href="{{ route('block.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_block')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                block_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('block.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
