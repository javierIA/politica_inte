...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('BoxTypeController','importData'))
                <a href="#" data-href="{{ route('box_type.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('BoxTypeController','exportData'))
        ...
        <tr>
            ...
            <th>{{trans('admin.box_type_name')}}</th>
            <th>{{trans('admin.quantity_per_box')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('BoxTypeController','create'))
        <a href="{{ route('box_type.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_box_type')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var box_type_table = $("#table-2");

                // Initialize DataTable
                box_type_table.DataTable( {
                    "language":{
                ...
                box_type_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    box_type_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('box_type.exportData', ['type1', 'items']) }}'.replace('type1', type);
                    if (items.length > 0)
