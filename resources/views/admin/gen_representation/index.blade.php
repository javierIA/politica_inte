...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('GenRepresentationController','importData'))
                <a href="#" data-href="{{ route('gen_representation.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('GenRepresentationController','exportData'))
        ...
        <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
            ...
            <tr>
                ...
                <th>{{trans('admin.fed_district')}}</th>
                <th>{{trans('admin.loc_district')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('GenRepresentationController','create'))
        <a href="{{ route('gen_representation.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_gen_representation')}}">
...
@section('script_down')
    <script type="text/javascript">
        function assignBoxes(id){
            var route = [id].reduce(_r, "{!! route('gen_representation.assignBoxes','%s') !!}");
            window.location.replace(route);
        }

        (function($){
            $(window).on('load',function(){
                var gen_representation_table = $("#table-2");

                // Initialize DataTable
                gen_representation_table.DataTable( {
                    "language":{
                    ...
                    "serverSide": true,
                    "ajax":{
                        "url": "{{route('gen_representation.filter')}}",
                        "dataType": "json",
                        "type": "POST",
                ...
                gen_representation_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    gen_representation_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('area.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
