...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('SystemFunctionController','importData'))
                <a href="#" data-href="{{ route('system_function.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('SystemFunctionController','exportData'))
    ...
    <table class="table table-bordered table-striped datatable" id="table-2" style="width: 100%">
        ...
        <tr>
            ...
            <th>{{trans('admin.system_function_name')}}</th>
    ...
    <br/>
    @if(auth()->user()->get_Permission('SystemFunctionController','create'))
        <a href="{{ route('system_function.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_system_function')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var system_function_table = $("#table-2");

                // Initialize DataTable
                system_function_table.DataTable( {
                    "language":{
                    ...
                    "serverSide": true,
                    "ajax":{
                        "url": "{{route('system_function.filter')}}",
                        "dataType": "json",
                        "type": "POST",
                ...
                system_function_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    system_function_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('system_function.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
