...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('AddressController','importData'))
                <a href="#" data-href="{{ route('address.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('AddressController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('AddressController','create'))
        <a href="{{ route('address.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_address')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                ...
                address_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $('.exportData').click(function(e){
                    ...
                    $.each(its, function( i, v ) {
                        items.push(v.id_address);
                    });
                    var type = $(this).data('type');
                    var url = '{{ route('address.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
