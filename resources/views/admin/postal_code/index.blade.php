...
@section('content')
    ...
    <div class="rcorners table-download" style="margin-top: 10px">
        <div class="icon" style="top: 15px; right: 20px;">
            @if(auth()->user()->get_Permission('PostalCodeController','importData'))
                <a href="#" data-href="{{ route('postal_code.importData') }}" class="importData" data-toggle="modal" data-target="#myModalImportData" style="color: grey" title="{{trans('admin.importData')}}"> <i class="fa fa-upload"></i></a>
            @endif
            @if(auth()->user()->get_Permission('PostalCodeController','exportData'))
    ...
    <br/>
    @if(auth()->user()->get_Permission('PostalCodeController','create'))
        <a href="{{ route('postal_code.create') }}" class="btn btn-primary btn-sm" title="{{trans('admin.add_postal_code')}}">
    ...
    <script type="text/javascript">
        (function($){
            $(window).on('load',function(){
                var postal_code_table = $("#table-2");

                // Initialize DataTable
                postal_code_table.DataTable( {
                    "language":{
                ...
                postal_code_table.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                ...
                $( "#filter-form" ).submit(function( event ) {
                    ...
                    postal_code_table.DataTable().ajax.reload();
                });
                ...
                $('.exportData').click(function(e){
                    ...
                    var url = '{{ route('postal_code.exportData', ['type', 'items']) }}'.replace('type', type);
                    if (items.length > 0)
